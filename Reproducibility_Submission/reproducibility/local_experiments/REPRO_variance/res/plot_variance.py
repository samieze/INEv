...
for k in toRemove:     
    for var in [0,10,20,30,50,70]:
        try:
            del data[var][k]
        except KeyError: #data to delete isn't present anyway (this is a defensive-programming workaround, this exception will never be thrown if all experiments run successfully) #fixme
            pass
...
plt.savefig("figs/Fig_11a_variance", format = 'pdf',  bbox_inches='tight')
