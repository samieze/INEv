...
def optimisticTotalRate(projection): # USE FILTERED RATE FOR ESTIMATION 
     if projection in projrates.keys():
           return projrates[projection][1] * getNumETBs(projection) #TODO change
                        
     elif isinstance(projection,Tree):
...
def returnPartitioning(proj, combi, *args):
    ...
    if myevents:
        res = isPartitioning(myevents[0], combi, proj)
...
#            else:
# =============================================================================
               additional = optimisticTotalRate(i)              
               mysum += additional #  len(returnETBs(projection, network))
     
       mysum -= optimisticTotalRate(element)

       if not proj in wl:
           mysum += optimisticTotalRate(proj) # additional constraint about ratio of partitioning event type and outputrate of projection
