...
def main():
    ...
    print("centralCosts: " + str(ccosts[0]), ccosts[1] )
    ...
    new = False
    try:
        f = open("res/"+str(filename)+"_Inev.csv")   
    except FileNotFoundError:
        new = True           
        
    with open("res/"+str(filename)+"_Inev.csv", "a") as result:
    ...
    with open('ExperimentResults',  'wb') as ExperimentID_file:
        pickle.dump([ID,mycosts,ccosts[0], networkParams[0],  networkParams[3], filename], ExperimentID_file)    
