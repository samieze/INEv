...
def unredundantcombo_noOverlap(combolist, projkey):  
    """ check if a combination is redundant """
    notredundant = True
    doubles = getdoubles_key(projkey)
    combolist = [x for x in combolist if not x in doubles]
    combistring = "".join(combolist)    
    for i in set(combistring):
        if combistring.count(i)>1:
            return False
    return notredundant  
    ...
    for curcom in newcurcoms:
            ...
            for critops in criticalops:
                if critops[0] in curcom:
                    ...
                    if not unredundantcombo_noOverlap(curcomkeys, myprojkey):
      ...
      for mysubop in global_subop_mapping[mysubopkey]:                     
                   ...
                   for i in range(1, len(filter_numbers(mysubopkey))):
                       ...
                       for k in range(len(iset)):                       
                                ...
                                if (projkey == mysubopkey or projkey=="".join(sorted(list(set(filter_numbers(mysubopkey)))))) and unredundantcombo_noOverlap(curcom, mysubopkey):        
...
for query in projections_of_matches.keys():    
    ...
    #del projections_of_matches[query]     #TODO: this Line is problematic
