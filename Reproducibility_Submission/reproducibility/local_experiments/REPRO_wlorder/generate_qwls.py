...
with open('current_wl',  'rb') as  wl_file:
        wl_from_file = pickle.load(wl_file)    
...
#def generateWorkload_all(count,length)
#generateQ with Kleene and Negation
...
def main():
    ...
    if count <=  5:
        ...
        wl =  wl_from_file
        dif = count - len(wl_from_file)
        new_wl = makeLongBalanced(dif, length)  
        wl = new_wl + wl 
    #if negated:
    ...
    #wl = [SEQ(PrimEvent('B'), PrimEvent('G'), PrimEvent('D'),PrimEvent('F'))]
    ...
    #wl = [SEQ(PrimEvent('A'), AND(PrimEvent('B'), PrimEvent('I')),PrimEvent('E')), AND(PrimEvent('E'), PrimEvent('F'), PrimEvent('C'),PrimEvent('H'))] #final 3 bikecity
    ...
    #wl = [AND(NSEQ(PrimEvent('B'), KL( PrimEvent('D')), PrimEvent('C')), NSEQ(PrimEvent('A'), KL(PrimEvent('E')), KL(PrimEvent('F'))))]
    #wl = [AND(SEQ(PrimEvent('B'), KL( PrimEvent('D')), PrimEvent('C')), SEQ(PrimEvent('A'), KL(PrimEvent('E')), KL(PrimEvent('F'))))]
    #wl = [SEQ(KL( PrimEvent('A')), PrimEvent('B'))]
