...
def inputcosts(projection): #input rate of projection
    ...
    for myinput in mycombi[projection]:
        if len(myinput) >1 :
            costs +=  myprojrates[myinput] *  prod(instances[x] for x in myinput.leafs())
        else:
...
def main():
    ...
    new = False
    try:
            f = open("../adaptivity_selectivity_change/selectivity_change.csv")   
    except FileNotFoundError:
            new = True           
            
    with open("../adaptivity_selectivity_change/selectivity_change.csv", "a") as result:
