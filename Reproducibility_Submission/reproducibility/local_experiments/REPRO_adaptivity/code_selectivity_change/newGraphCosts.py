...
def main():
    ...
    for k in myplan.projections:    
        ...
        if len(k.name.sinks)>1:
            ...
            MSTypes.append(getparttype(myplan.getProjection(k.name.name)))
    ...
    schema = ["ID", "TransmissionRatioINEV", "Change", "TransmissionRatioChange"] 
    
    myResult = [results[0], results[1], percentage,  costs/results[2]] 
