...
def getMultiNodeRoutes(projection,mycombi):
    mydict = {}
...
def inputcosts(projection, mycombi, myprojrates): #input rate of projection
    costs = 0   
...
def main(): 
    ...
    # 1. beim durchiterieren durch die projektionen, für alle multi-node placements subgraphen pro input etb rausrechnen
    ...
    for k in myplan.projections:    
        ...
        if len(k.name.sinks)>1:
            ...
            multinodeRoutes[k.name.name]= getMultiNodeRoutes(k.name.name, mycombi)
    
    #print(multinodeRoutes)
    ...
    # 2. projrates anpassen auf projrates bei durchreichen der inputs (inputrate)
    # 3. alle multi-node projektionen die eine neue outputrate bekommen haben sind automatisch kaputt
    ...
    for projection in sorted(myprojs, key = lambda x: levels[x]):
        if not projection in wl:
            if myprojrates[projection] * getNumETBs(projection) > inputcosts(projection,mycombi, myprojrates): 
                brokenProjs += [projection]
                if projection in multinode:
                    brokenMNplacements  += [projection]
                myprojrates[projection] = inputcosts(projection,mycombi,myprojrates) /  getNumETBs(projection)
    ...
    # 4. zusätzliche kaputte multi-node placements berechnen
    ...
    # 5. für jedes kaputte multi-node placement:  setze subgraph auf shortest path für jedes input etb
    for projection in sorted(brokenMNplacements, key = lambda x: levels[x]):
        ...
        if not projection in wl:
            for etb in myplan.getProjection(projection).name.spawnedInstances:            
                # füge shortest path zum neuen single sink für ex part eventtypen hinzu
                myRouting[etb] = getSingleSinkRouting(sources[etb], sink)
                # füge source hinzu
                sources[etb] = sink 
                
    # vereinige für jedes etb alle pfade 
    newPath = {}
    ...
    new = False
    try:
           f = open("../adaptivity_swap_rates/results_adaptivity_"+str(skew)+".csv")   
    except FileNotFoundError:
           new = True           

    with open("../adaptivity_swap_rates/results_adaptivity_"+str(skew)+".csv", "a") as result:
