...
pwd
cat GeneratedSourceFiles/DCEP.AmbrosiaNodeAPIGenerated/latest/DCEP.AmbrosiaNodeAPIGenerated.csproj
sed -i 's_<Platforms>x64</Platforms>_<Platforms>ARM</Platforms><PlatformTarget>ARM</PlatformTarget>_g' GeneratedSourceFiles/DCEP.AmbrosiaNodeAPIGenerated/latest/DCEP.AmbrosiaNodeAPIGenerated.csproj
#read -p "Add the thing"
cat GeneratedSourceFiles/DCEP.AmbrosiaNodeAPIGenerated/latest/DCEP.AmbrosiaNodeAPIGenerated.csproj
set +x
...
set -x
BUILDITLINUX="dotnet publish -o publish -c $CONF -f $FMWK -r linux-arm"
