...
def getDecomposed(primEvents, projection): #TODO: very primitive variant which only sends primitive events filtered by single selectivity, 
    mysum = 0
    for event in  projection.leafs():
        if not event in primEvents:
...
def getDecomposedTotal(primEvents, projection): #TODO: very primitive variant which only sends primitive events filtered by single selectivity, 
    mysum = 0
...
# print("H")
# for i in projrates.keys():
#      print(str(i) + " : " + str(projrates[i][1] * getNumETBs(i)))
#      for k in i.leafs():
#          print(str(k) + " : " + str(getDecomposedTotal(k, i)))
