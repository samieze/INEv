...
def main():
    ...
    for path in myargs.inputs: # schemas are schemas are equal or overlap
        df = pd.read_csv(path)
    ...
    for i in range(len(mydata)):
        ...
        #plt.plot(myX,dfoOP.groupby(X_Column)[Y_Column].mean(),marker="o", label = "oOP")
    plt.legend()
    ...
    plt.savefig("plots/"+myargs.outname, format = 'pdf',  bbox_inches='tight')
    plt.show()
