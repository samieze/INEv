...
def main():
    ...
    if len(sys.argv) > 1:
         # writeExperimentData = int(sys.argv[1]) # TODO for SHARES WINDOW STUFF
         filename = sys.argv[1]
    # if len(sys.argv) > 1:
    #     
    # if len(sys.argv) > 2:
    ...
    if writeExperimentData:
        with open('ExperimentResults',  'wb') as ExperimentID_file: 
         #   pickle.dump([ID,costs], ExperimentID_file)  # write only INEv Costs and ID for timewindow exp
             pickle.dump([ID,mycosts,ccosts[0]], ExperimentID_file) # for adaptivity_selectivity
             print("writing: ", [ID,mycosts,ccosts[0]])
    #for q in wl:
    #    print(projrates[q][1])
