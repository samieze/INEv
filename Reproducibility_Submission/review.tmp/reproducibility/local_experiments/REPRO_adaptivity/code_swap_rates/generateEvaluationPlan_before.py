...
for i in evaluationDict.keys(): #add filtered projections to evaluation Dict of nodes processing filtered inputs
     ...
     evaluationDict[i] += list(set(projectionsToAdd)) 
     myFilteredInputs[i] = projectionsToAdd             
     #evaluationDict[i] = list(set(evaluationDict[i])) # TODO: this is problematic, as a projection might be placed on a node and at the same time input to the node in its filtered version
...
def processingRules_Diamonds(i):  
    ...
    if evaluationDict[i]:
        ...
        print(evaluationDict[i])
        print(myFilteredInputs[i])
        ...
        for projection in evaluationDict[i]:  
                ...
                if projection in MSPlacements.keys():
                    ...
                    partType =  ""
                if projection in myFilteredInputs[i]: #use combination made of leafs when filtered and  processing based on filterered inputs
                    mycombination = strToProj(projection,myquery).leafs()
                    partType = ""
                    mySelRate  = singleSelectivities[getKeySingleSelect(filterDict[projection][0],strToProj(projection, myquery))]  # TODO is it really the single selectivity of the filter?
                else:
                ...
                myDiamonds = getMiniDiamonds(strToProj(projection,myquery), partType, mycombination)              
...
def processingRulesCentral_Diamonds(i):
    ...
    if evaluationDict[node]:
        ...
        if i == csource:
            for query in wl:
                myDiamonds = getMiniDiamonds(query, "", query.leafs(), "", 100)
                # for diamond in myDiamonds:
