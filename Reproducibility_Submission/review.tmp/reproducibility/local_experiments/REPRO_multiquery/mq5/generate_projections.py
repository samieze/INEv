...
def NEW_isPartitioning_customRates(element, combi, proj, myrates):
       ...
       # elif proj.get_original(wl) in wl and element in list(map(lambda x: str(x), proj.get_original(wl).kleene_components())):
       #     
       ...
       for i in [x for x in combi if not x == element]:            
           if i in rates.keys():        
              ...
              mysum += additional              
               ...
               additional = projrates[i][1] * getNumETBs(i)             # BEFORE, 8.2.
              # print("old additional: " + str(i) + " " + str(additional))
              # additional = optimisticTotalRate_single(i)
              # print("new additional: " + str(i) + " " + str(additional))
               mysum += additional
       ...
       for i in [x for x in combi if not x == element]:            
           if i in rates.keys():        
              ...
              mysum += additional              
               ...
               additional = getDecomposedTotal(getMaximalFilter(myprojFilterDict, i), i)              # BEFORE, 8.2.
              # print("old additional: " + str(i) + " " + str(additional))
              # additional = optimisticTotalRate_single(i)
              # print("new additional: " + str(i) + " " + str(additional))
               mysum += additional
...
for q in wl:
     print(q, totalRate(q)) # USED TO FIND SUITABLE INPUTS FOR PARALLELIZATION
