...
def generate_eventrates(eventskew, numb_eventtypes):
    while True:
        eventrates = np.random.zipf(eventskew, numb_eventtypes)
        if np.all(np.multiply(eventrates, 10, dtype=np.longlong) < np.iinfo(np.longlong).max) and np.sum(eventrates, dtype=np.longlong) < np.iinfo(np.longlong).max:
            return eventrates
...
def main():
    ...
    #evenetrates =  [0.5, 6, 1, 136, 1000, 250, 0.5, 30, 60] # citibike 20
